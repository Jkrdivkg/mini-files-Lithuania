...
function OpenActionMenuInteraction(target)
	...
	table.insert(elements, {label = ('Apsirengti'), value = 'ubie'})
	table.insert(elements, {label = ('Virsutine dalis'), value = 'tul'})
	table.insert(elements, {label = ('Apatine dalis'), value = 'spo'})
	table.insert(elements, {label = ('Batai'), value = 'but'})
...
RegisterCommand("virsus", function()

	TriggerEvent('smerfikubrania:koszulka')
	
	end)
	
RegisterCommand("apacia", function()

	TriggerEvent('smerfikubrania:spodnie')
	
	end)
	
RegisterCommand("batai", function()

	TriggerEvent('smerfikubrania:buty')
	end)
	
RegisterNetEvent('aprengti')
AddEventHandler('aprengti', function()
	ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
	TriggerEvent('skinchanger:loadSkin', skin)
	end)
end)
